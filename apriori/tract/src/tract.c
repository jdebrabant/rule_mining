...
/*----------------------------------------------------------------------
  Constants
----------------------------------------------------------------------*/
static WIIT   SENT = { -1, 0 }; /* sentinel for item instance arrays */
...
{                               /* --- create an item base */
  ...
  if (mode & IB_WEIGHTS) {      /* if items carry weights */
    base->tract = x = (WTRACT*)malloc(sizeof(WTRACT)
                               +size *sizeof(WIIT));
    if (x) { x->size = x->wgt = 0; x->items[0] = x->items[size] = SENT;}}
  else {                        /* if items do not carry weights */
    base->tract = t = (TRACT*) malloc(sizeof(TRACT)
                               +size *sizeof(int));
    if (t) { t->size = t->wgt = 0; t->items[0] = t->items[size] = -1; }
...
{                               /* --- add an item to the set */
  ...
  item = (ITEM*)idm_add(base->idmap, name, sizeof(ITEM));
...
{                               /* --- add an item to transaction */
  int   n;                      /* size of the transaction buffer */
  ITEM  *item;                  /* to access the item data */
  ...
  if (t->size >= n) {           /* if the transaction buffer is full */
    ...
    t->items[n] = -1; base->tract = t; base->size = n;
...
{                               /* --- finalize transaction buffer */
  ...
  t->items[t->size] = -1;       /* store a sentinel at the end */
...
{                               /* --- read insertion penalties */
  ...
  if (*b || (e == b) || (p > 1)) return base->err = E_PENALTY;
...
{                               /* --- read a transaction */
  ...
  ITEM   *item;                 /* to access the item data */
  WIIT   *wit;                  /* to access the item data (weighted) */
  TRACT  *t;                    /* to access the transaction */
  ...
  do {                          /* read the transaction items */
    ...
    if (item->idx >= base->idx){/* if the item is already contained, */
      ...
      if (!(mode & TA_DUPLICS)){/* if to ignore duplicates */
        ...
        &&  trd_istype(trd, trd_last(trd), TA_WGTSEP)) {
          ...
          if (d <= TRD_ERR)  return base->err = E_FREAD;
          z = strtod(trd_field(trd), &b);
          if (*b || (z < 0)) return base->err = E_ITEMWGT;
    ...
    if (base->mode & IB_WEIGHTS) { /* if the items carry weights */
      if (x->size >= n) {       /* if the transaction buffer is full */
        n += (n > BLKSIZE) ? (n >> 1) : BLKSIZE;
        x  = (WTRACT*)realloc(x, sizeof(WTRACT) +n *sizeof(WIIT));
        if (!x) return base->err = E_NOMEM;
        x->items[n] = SENT; base->tract = x; base->size = n;
      ...
      if (!trd_istype(trd, trd_last(trd), TA_WGTSEP))
        ...
        if (d <= TRD_ERR) return base->err = E_FREAD;
        wit->wgt = (float)strtod(trd_field(trd), &b);
        if (*b || (wit->wgt < 0)) return base->err = E_ITEMWGT;
...
{                               /* --- recode items w.r.t. frequency */
  ...
  WIIT   *a, *b;                /* to traverse the items */
...
{                               /* --- show a transaction */
  ...
  for (k = 0; k < t->size; k++){/* traverse the items */
    if (k > 0) fputc(' ', stdout);
    if (base) printf("%s/", ib_xname(base, t->items[k]));
...
int wi_cmp (const WIIT *a, const WIIT *b)
...
static void _wi_rec (WIIT *wia, int n)
{                               /* --- recursive part of item sorting */
  WIIT *l, *r, t;               /* exchange positions and buffer */
  ...
  do {                          /* sections sort loop */
    ...
    if (n > m) {                /* if right section is smaller, */
      if (m >= TH_INSERT)       /* but larger than the threshold, */
        _wi_rec(l, m); }        /* sort it by a recursive call, */
    else {                      /* if the left section is smaller, */
      if (n >= TH_INSERT)       /* but larger than the threshold, */
        _wi_rec(wia, n);        /* sort it by a recursive call, */
...
void wi_sort (WIIT *wia, int n)
{                               /* --- sort an transaction item array */
  int  k;                       /* size of first section */
  WIIT *l, *r;                  /* to traverse the array */
  WIIT t;                       /* exchange buffer */
  ...
  if (n < TH_INSERT)            /* if fewer elements than threshold */
    ...
    _wi_rec(wia, n);            /* call the recursive function */
...
void wi_reverse (WIIT *wia, int n)
...
int wi_unique (WIIT *wia, int n)
{                               /* --- make item array unique */
  WIIT *s, *d;                  /* to traverse the item array */
...
{                               /* --- create a transaction */
  ...
  t = (WTRACT*)malloc(sizeof(WTRACT) +size *sizeof(WIIT));
  if (!t) return NULL;          /* allocate a new transaction */
...
{                               /* --- clone a transaction */
  ...
  WIIT   *d; const WIIT *s;     /* to traverse the items */

  c = (WTRACT*)malloc(sizeof(WTRACT) +t->size *sizeof(WIIT));
...
{                               /* --- add an item to a transaction */
  WIIT *d;                      /* destination for the item */
...
{                               /* --- count item occurrences */
  ...
  WIIT   *p;                    /* to traverse the transaction items */
  ...
  if (bag->mode & IB_WEIGHTS) { /* if the items carry weights */
    for (i = bag->cnt; --i >= 0; ) {
      x = bag->tracts[i];       /* traverse the transactions */
...
void _recode (TABAG *bag, int *map)
...
int tbg_recode (TABAG *bag, int min, int max, int dir)
{                               /* --- recode items in transactions */
  ...
  min = ib_recode(bag->base, min, max, dir, map);
  _recode(bag, map);            /* recode items and transactions */
  ...
  if (bag->mode & IB_WEIGHTS) { /* if the items carry weights */
    for (n = bag->cnt; --n >= 0; ) {
      ...
      if (marks) {              /* if item markers are given */
        ...
        x->items[k] = SENT;     /* and a (new) sentinel */
        for (a = b = x->items; a->id >= 0; a++)
...
{                               /* --- mirror all transactions */
  ...
  else {                        /* if the items have unit weights */
    for (i = 0; i < bag->cnt; i++) {
...
{                               /* --- sort a transaction bag */
  assert(bag);                  /* check the function arguments */
  if (bag->mode & IB_WEIGHTS) { /* if the items carry weights */
    if (heap) ptr_heapsort(bag->tracts, bag->cnt, wta_cmp, NULL);
    else      ptr_qsort   (bag->tracts, bag->cnt, wta_cmp, NULL); }
  else {                        /* if the items have unit weights */
    if (heap) ptr_heapsort(bag->tracts, bag->cnt,  ta_cmp, NULL);
    else      ptr_qsort   (bag->tracts, bag->cnt,  ta_cmp, NULL);
...
{                               /* --- sort a transaction bag */
  assert(bag);                  /* check the function arguments */
  if (bag->mode & IB_WEIGHTS) { /* if the items carry weights */
    if (heap) ptr_heapsort(bag->tracts, bag->cnt, wta_cmpsz, NULL);
    else      ptr_qsort   (bag->tracts, bag->cnt, wta_cmpsz, NULL); }
  else {                        /* if the items have unit weights */
    if (heap) ptr_heapsort(bag->tracts, bag->cnt,  ta_cmpsz, NULL);
    else      ptr_qsort   (bag->tracts, bag->cnt,  ta_cmpsz, NULL);
  }                             /* sort the transactions */
  if (dir < 0) ptr_reverse(bag->tracts, bag->cnt);
...
{                               /* --- show a transaction bag */
  ...
  assert(bag);                  /* check the function argument */
  for (i = 0; i < bag->cnt; i++) {
    if (bag->mode & IB_WEIGHTS) /* traverse the transactions */
...
int _create (TANODE *node, TRACT **tracts, int cnt, int index)
{                               /* --- recursive part of tat_create() */
  ...
  child[n].item = -1;           /* create a child node array and */
...
{                               /* --- create a transactions tree */
  ...
  tree->suffix[0] = -1;         /* init. the empty trans. suffix */
  return tree;                  /* return the created tree */
}  /* tat_create() */

/*--------------------------------------------------------------------*/

void _delete (TANODE *node)
...
static int _nodecnt (const TANODE *node)
{                               /* --- count the nodes */
  ...
  assert(node && (node->max > 0)); /* check the function argument */
  for (child = (TANODE*)node->data; child->item >= 0; child++)
    if (child->max > 0) n += _nodecnt(child);
  return n;                     /* recursively count the nodes */
}  /* _nodecnt() */             /* return number of nodes in tree */

/*--------------------------------------------------------------------*/

int tat_size (const TATREE *tree)
{ return (tree->root.max > 0) ? _nodecnt(&tree->root) : 1; }
...
{                               /* --- filter a transaction tree */
  ...
  tbg_reduce(bag);              /* and reduce trans. to unique ones */
  if (_create(&tree->root, (TRACT**)bag->tracts, bag->cnt, 0) == 0)
    return 0;                   /* recreate the transaction tree */
  _delete(&tree->root);         /* on failure delete the nodes */
  tree->root.max  = tree->root.wgt = 0;
...
static void _show (TANODE *node, ITEMBASE *base, int ind)
...
{                               /* --- show a transaction tree */
  assert(tree);                 /* check the function argument */
  _show(tree->root, tbg_base(tree->bag), 0);
...
TANODE* _create (TRACT **tracts, int cnt, int index)
{                               /* --- recursive part of tat_create() */
  ...
  for (--cnt; --n >= 0; cnt = i) {  /* traverse the different items */
    ...
    cnds[n] = _create(tracts +i+1, cnt-i, index+1);
  ...
  while (++n < node->size) _delete(cnds[n]);
...
static int _nodecnt (const TANODE *root)
{                               /* --- count the nodes */

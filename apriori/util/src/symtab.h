...
#define ST_STRFN    st_strcmp, NULL, st_strhash, st_strsize
#define ST_INTFN    st_intcmp, NULL, st_inthash, st_intsize
...
typedef struct _ste {           /* --- symbol table element --- */
  struct _ste *succ;            /* successor in hash bin */
...
extern SYMTAB*      st_create  (int init, int max,
                                CMPFN cmpfn, void *data, HASHFN hashfn,
                                SIZEFN sizefn, OBJFN delfn);
...
----------------------------------------------------------------------*/
#ifdef IDMAPFN
extern IDMAP*       idm_create (int init, int max,
                                CMPFN cmpfn, void *data, HASHFN hashfn,
                                SIZEFN sizefn, OBJFN delfn);
...
#define idm_add(m,n,s)    st_insert(m,n,0,s)

/*----------------------------------------------------------------------
  ...
  History : 1996.02.14 file created
            ...
            2003.08.21 option -j added (heap sort for transactions)
...
#define VERSION     "version 5.49 (2011.08.03)        " \
                    "(c) 1996-2011   Christian Borgelt"
...
static int      *map    = NULL; /* identifier map for filtering */
#endif
...
int apriori (TABAG *tabag, int mode, int supp,
             int eval, int agg, double thresh, double minimp,
             int prune, double filter, ISREPORT *isrep)
{                               /* --- apriori algorithm */
  int     i, k, n;              /* loop variables, buffers */
  int     size, max;            /* current/maximal item set size */
  clock_t t, tt, tc, x;         /* timers for measurements */
  APRIORI data = { NULL, NULL, NULL }; /* apriori execution data */

  /* --- create transaction tree --- */
  tt = 0;                       /* init. the tree construction time */
  if (mode & AV_TATREE) {       /* if to use a transaction tree */
    t = clock();                /* start the timer for construction */
    data.tatree = tat_create(tabag);   /* create a transaction tree */
    if (!data.tatree) return error(&data);
    tt = clock() -t;            /* note the time for the construction */
  }                             /* of the transaction tree */

  /* --- create item set tree --- */
  if ((isr_mode(isrep) & (ISR_CLOSED|ISR_MAXIMAL))
  || ((eval > IST_NONE) && (eval < IST_LDRATIO)))
    mode &= ~IST_PERFECT;       /* if individual set counters needed */
  t = clock(); tc = 0;          /* start the timer for the search */
  data.istree = ist_create(tbg_base(tabag), mode, supp, INT_MAX, 1);
  if (!data.istree) return error(&data);
  ist_seteval(data.istree, eval, agg, thresh, minimp, prune);
  eval &= ~IST_INVBXS;          /* create an item set tree */

  /* --- check item subsets --- */
  max = isr_max(isrep);         /* get the maximum item set size */
  if ((k = tbg_max(tabag)) < max) max = k;
  n = tbg_itemcnt(tabag);       /* get the number of items */
  data.map = (int*)malloc(n *sizeof(int));
  if (!data.map) return error(&data);   /* create a filter map */
  for (i = n; 1; ) {            /* traverse the item set sizes */
    size = ist_height(data.istree);/* get the current item set size */
    if (size >= max) break;     /* abort if maximal size is reached */
    if ((filter != 0)           /* if to filter w.r.t. item usage */
    && ((i = ist_check(data.istree, data.map)) <= size))
      break;                    /* check which items are still used */
    k = ist_addlvl(data.istree);/* add a level to the item set tree */
    if (k < 0) return error(&data);
    ...
    &&   (i < n) && (i *(double)tt < filter *n *tc))) {
      ...
      if (data.tatree) {        /* if a transaction tree was created */
        if (tat_filter(data.tatree, size+1, data.map, 1) != 0)
          return error(&data);} /* filter the transaction tree */
      else {                    /* if there is only a transaction bag */
        tbg_filter(tabag, size+1, data.map, 0);
        tbg_sort  (tabag, 0, 1);
        tbg_reduce(tabag);      /* remove unnec. items and trans. */
      }                         /* and reduce trans. to unique ones */
      tt = clock() -x;          /* note the filter/rebuild time */
    }
    ++size;                     /* increment the item set size */
    x = clock();                /* start the timer for counting */
    if (data.tatree) ist_countx(data.istree, data.tatree);
    else             ist_countb(data.istree, tabag);
    tc = clock() -x;            /* count the transaction tree/bag */
  }                             /* and compute the new counting time */
  free(data.map);               /* delete the filter map */

  /* --- filter found item sets --- */
  k = isr_mode(isrep) & (ISR_CLOSED|ISR_MAXIMAL);
  if ((k == ISR_CLOSED) || (k == ISR_MAXIMAL))
    ist_mark(data.istree, k | ((prune < 0) ? IST_EVAL : 0));

  /* --- report found item sets --- */
  if      (eval == IST_LDRATIO) /* if to compute add. evaluation */
    isr_seteval(isrep, isr_logrto, NULL,       thresh, +1);
  else if (eval >  IST_NONE)    /* set the add. evaluation function */
    isr_seteval(isrep, ist_evalx, data.istree, thresh, re_dir(eval));
  ist_init(data.istree);        /* initialize the extraction and */
  ist_report(data.istree,isrep);/* report item sets recursively */
  return 0;                     /* return 'ok' */
}  /* apriori() */
...
{                               /* --- main function */
  ...
  double  minimp   = -DBL_MAX;  /* minimum increase of measure value */
  ...
  #endif  /* #ifndef QUIET */
  /* free option characters: [A-Z]\[CIST] */
  ...
  if ((target > ISR_ALL)        /* if individual set counters needed */
  || ((eval > IST_NONE) && (eval < IST_LDRATIO)))
    mode &= ~IST_PERFECT;       /* remove perfect extension pruning */
  if (target <= ISR_MAXIMAL) {  /* remove rule specific settings */
  ...
  if (tree) {                   /* if to use a transaction tree */
    ...
    tatree = tat_create(tabag); /* create a transaction tree */
    if (!tatree) error(E_NOMEM);
    if (filter == 0) {          /* if not to filter items, */
      tbg_delete(tabag, 0);     /* delete the transaction bag */
      tabag = NULL;             /* (redundant data storage) */
    }                           /* print the size of the tree */
    MSG(stderr, "[%d node(s)]", tat_size(tatree));
    MSG(stderr, " done [%.2fs].\n", SEC_SINCE(t));
    tt = clock() -t;            /* note the time for the construction */
  }                             /* of the transaction tree */
  ...
  /* --- create item set tree --- */
  t = clock(); tc = 0;          /* start the timer for the search */
  istree = ist_create(ibase, mode, (int)ceil(supp), (int)smax, conf);
  if (!istree) error(E_NOMEM);  /* create an item set tree */
  ist_seteval(istree, eval|invbxs, agg, thresh, minimp, prune);
  ...
  for (i = n; 1; ) {            /* traverse the item set sizes */
    ...
    &&   (i < n) && (i *(double)tt < filter *n *tc))) {
      ...
      if (tatree) {             /* if a transaction tree was created */
        if (tat_filter(tatree, size+1, map, heap) != 0)
          error(E_NOMEM); }     /* filter the transaction tree */
      else {                    /* if there is only a transaction bag */
        tbg_filter(tabag, size+1, map, 0);
        tbg_sort  (tabag, 0, heap);
        tbg_reduce(tabag);      /* remove unnec. items and trans. */
      }                         /* and reduce trans. to unique ones */
      tt = clock() -x;          /* note the filter/rebuild time */
    }
  ...
  /* --- filter found item sets --- */
  if ((target == ISR_CLOSED) || (target == ISR_MAXIMAL)) {
  ...
  if (eval == IST_LDRATIO) report |= ISR_LOGS;
  if ((target == ISR_CLOSED) || (target == ISR_MAXIMAL))
    report |= ISR_NOEXP;        /* combine the report mode flags */
  isrep = isr_create(ibase, report, -1, hdr, sep, imp);
  ...
  if ((target <= ISR_MAXIMAL)   /* if to find frequent item sets */
  &&  (dir == 0)) {             /* and not to sort them by size */
    if      (eval == IST_LDRATIO)  /* if to compute add. evaluation */
      isr_seteval(isrep, isr_logrto,  NULL, thresh, +1);
    else if (eval >  IST_NONE)  /* set the add. evaluation function */
      isr_seteval(isrep, ist_evalx, istree, thresh, re_dir(eval));
    ist_report(istree, isrep);} /* recursively report the item sets */
  else if (target <= ISR_MAXIMAL){ /* if to find frequent item sets */
    items = (int*)malloc((size+1) *sizeof(int));
    if (!items) error(E_NOMEM); /* create an item set buffer */
    while (1) {                 /* extract item sets from the tree */
      k = ist_set(istree, items, &frq, &thresh);
      if (k < 0) break;         /* get the next frequent item set */
      isr_direct(isrep, items, k, frq, 0, thresh);
    }                           /* report the extracted item set */
    free(items); }              /* delete the item set buffer */
  else if (target == ISR_RULE){ /* if to find association rules, */
    items = (int*)malloc((size+1) *sizeof(int));
    if (!items) error(E_NOMEM); /* create an item set buffer */
    while (1) {                 /* extract rules from tree */
      k = ist_rule(istree, items, &frq, &body, &head, &thresh);
      if (k < 0) break;         /* get the next association rule */
      isr_rule(isrep, items, k, frq, body, head, thresh);
    }                           /* report the extracted ass. rule */
    free(items);                /* delete the item set buffer */
  }  /* if (target <= ISR_MAXIMAL) .. else .. */
  if (isr_close(isrep) != 0)    /* close the output file */
    error(E_FWRITE, isr_name(isrep));
  MSG(stderr, "[%d %s(s)] done ", isr_repcnt(isrep),

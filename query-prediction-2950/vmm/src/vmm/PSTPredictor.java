...
//    implements VMMPredictor
{
  ...
  public void learn(List<Integer> trainingSequence) {
PSTBuilder builder = new PSTBuilder(abSize);
    ...
    samples.init(new StringSampleIter(trainingSequence.toString()));*/
      SamplesInt samples = new SamplesInt();
      samples.AddSample(trainingSequence);
    pst = builder.build(samples, pMin, alpha, gamma, r, vmmOrder);
  }

  //public double predict(int symbol, CharSequence context) {
  public double predict(int symbol, List<Integer> context) {
    try {
  ...
  public double logEval(List<Integer> testSequence) {
    try {
      double pArr[] = new double[abSize];
      double eval = 0.0;
  ...
  public double logEval(List<Integer> testSequence, List<Integer> initialContext) {
    try {
      double pArr[] = new double[abSize];
      double eval = 0.0;
      ...
      for (int i = 0, sym = -1; i < initialContext.size(); ++i) {
        sym = (int) initialContext.get(i);
        pstPredictor.increment(sym);
      }

      for (int i = 0, sym = -1; i < testSequence.size(); ++i) {
        sym = (int) testSequence.get(i);
        pstPredictor.predict(pArr);
        eval += Math.log(pArr[sym]);
        pstPredictor.increment(sym);
      }

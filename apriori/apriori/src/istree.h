/*----------------------------------------------------------------------
  ...
  History : 1996.01.22 file created as istree.h
            ...
            1998.02.11 minval added to function ist_init
...
#define IST_ALL     ISR_ALL     /* all     frequent item sets */
#define IST_CLOSED  ISR_CLOSED  /* closed  frequent item sets */
#define IST_MAXIMAL ISR_MAXIMAL /* maximal frequent item sets */
#define IST_RULE    ISR_RULE    /* association rules */
#define IST_EVAL    ISR_WEIGHTS /* filter also with evaluation */
...
typedef struct _isnode {        /* --- item set node --- */
  ...
  struct _isnode *parent;       /* parent    node (one level up) */

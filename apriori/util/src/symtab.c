/*----------------------------------------------------------------------
  ...
  History : 1995.10.22 file created
            ...
            1998.09.01 bug in function _sort() fixed, assertions added
...
{                               /* --- string hash function */
  register const char  *p = s;  /* to traverse the key */
...
SYMTAB* st_create (int init, int max, CMPFN cmpfn, void *data,
                   HASHFN hashfn, SIZEFN sizefn, OBJFN delfn)
{                               /* --- create a symbol table */
  ...
  tab->sizefn = (sizefn) ? sizefn : st_strsize;
  tab->delfn  = delfn;
  tab->idsize = INT_MAX;
  tab->ids    = NULL;
...
{                               /* --- insert a symbol (name/key) */
  ...
  k = tab->sizefn(key);         /* get the size of the key/name */
  n = (STE*)malloc(sizeof(STE) +size +k);
  if (!n) return NULL;          /* allocate memory for new symbol */
  memcpy(n->key = (char*)(n+1) +size, key, k);
...
IDMAP* idm_create (int init, int max, CMPFN cmpfn, void *data,
                   HASHFN hashfn, SIZEFN sizefn, OBJFN delfn)
{                               /* --- create a name/identifier map */
  IDMAP *idm;                   /* created name/identifier map */

  idm = st_create(init, max, cmpfn, data, hashfn, sizefn, delfn);

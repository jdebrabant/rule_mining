...
public class TimeseriesTaskSimulator
{
    private Random rand;
    ...
    public static void main(String args[])
    {
		if(args.length != 6)
		{
			System.out.println("usage: java TimeseriesTaskSimulator <min x> <max x> <min y> <max y> <num x partitions> <num y partitions>\n"); 
		}
		
        try 
		{
            TimeseriesTaskSimulator task_simulator = new TimeseriesTaskSimulator(Integer.parseInt(args[0]), Integer.parseInt(args[1]), Integer.parseInt(args[2]),
			...
			//task_simulator.trim("task1.arff", "task1-trimmed.arff"); 
			//task_simulator.trim("task2.arff", "task2-trimmed.arff"); 
			//task_simulator.trim("task3.arff", "task3-trimmed.arff"); 
    ...
    public TimeseriesTaskSimulator(int minimum_x, int maximum_x, int minimum_y, int maximum_y, int number_of_partitions_x, int number_of_partitions_y)
    {
        rand = new Random();
        min_x = minimum_x;
        max_x = maximum_x;
        ...
        try
        {
			...
			/*
            // open 1 output file for each task
            out1 = new FileWriter("task1.arff");
            out2 = new FileWriter("task2.arff"); 
            out3 = new FileWriter("task3.arff");
            
         // set up relation section of arff file
            out1.write("@relation nyse-task1\n\n");
            out2.write("@relation nyse-task2\n\n");
            out3.write("@relation nyse-task3\n\n"); 
            
            // set up attribute section of arff file
            for(int i = 0; i <  num_partitions_x; i++)
            {
                for(int j = 0; j < num_partitions_y; j++)
                {
                    attribute = "@attribute "; 
                    attribute += "partition_" + i + "_" + j; 
                    attribute += " { TRUE, FALSE }\n"; 
                    
                    out1.write(attribute); 
                    out2.write(attribute); 
                    out3.write(attribute); 
                }
            }
    ...
    {
        //int partition_size_x = (max_x - min_x) / num_partitions_x; 
        //int partition_size_y = (max_y - min_y) / num_partitions_y; 
